...
def runnable_test_id(nodeid: str) -> str | None:
    """Keep concrete pytest classes; collapse only parameter variants."""
    if "::" not in nodeid:
        return None
    prefix, separator, last = nodeid.rpartition("::")
    return prefix + separator + last.split("[", 1)[0]
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=HERE / "rounds/round-01-freeze")
